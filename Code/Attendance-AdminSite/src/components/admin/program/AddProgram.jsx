import React, { useState, useContext, useEffect } from 'react';
import { withRouter } from 'react-router-dom';
import ApiRequest from '../ApiRequest';
import {AuthContext} from "../../../contexts/authContext";


function AddProgram(props) {
  const [authenticated, setAuthenticated]=useContext(AuthContext);
  const [id, setId] = useState('');
  const [name, setName] = useState('');
  const [department, setDepartment] = useState('');
  const [duration, setDuration] = useState('');
  const [savedDepartments, setSavedDepartments] = useState([]);

  useEffect(() => {
    async function getDepartments() {
      const requestDepartments = await ApiRequest.get('/department');
      setSavedDepartments(requestDepartments.data);
    };

    getDepartments();
  }, []);

  const submitForm = (e) => {
    e.preventDefault();
    const data = {
      id: id,
      name: name,
      dept: department,
      duration: duration
    }
    ApiRequest.post('/secret/add-program', data).then((response)=>{
      props.history.push("/home")
    }).catch((err)=>
    {
     if (err.response.status==401)
     alert("You must be logged in to perform this action")
     setAuthenticated(false)
    });;
  };

  return(
    <React.Fragment>
      <div className="card mx-auto my-5 text-center" style={{maxWidth: "20rem"}}>
        <div className="card-header">Add Program</div>
        <div className="card-body">
          <form>
            <input type="text" className="form-control mb-3" placeholder="Id" onChange={ e => setId(e.target.value) }></input>
            <input type="text" className="form-control mb-3" placeholder="Program" onChange={ e => setName(e.target.value) }></input>
            <select className="form-control mb-3" onChange={ e => setDepartment(e.target.value)}>
              <option style={{display:"none"}}>Department</option>
              { savedDepartments.map(dept =>
                <option value={dept.id} key={dept.id}>{dept.name}</option>
              )}
            </select>
            <input type="text" className="form-control mb-3" placeholder="Duration" onChange={ e => setDuration(e.target.value) }></input>
           <button type="submit" className="btn btn-primary btn-block" onClick={ submitForm }>Add Program</button>
          </form>
        </div>
      </div>
    </React.Fragment>
  )
};

export default withRouter(AddProgram);